...
const diseasesDatabase = [
    ...
    { name: "Viêm xoang", group: "Hô hấp", keywords: ["đau trán", "nghẹt mũi", "chảy mũi", "xoang"], risk: "Trung bình", advice: "Xông hơi nước ấm và vệ sinh mũi." },
    ...
    /* ================= TIÊU HÓA (16-35) ================= */
    { name: "Đau dạ dày", group: "Tiêu hóa", keywords: ["đau bụng trên", "ợ chua", "buồn nôn", "bao tử"], risk: "Trung bình", advice: "Ăn uống đúng giờ, tránh đồ chua cay." },
    ...
    { name: "Nhiệt miệng", group: "Tiêu hóa", keywords: ["loét miệng", "đau niêm mạc", "nhiệt miệng"], risk: "Thấp", advice: "Bổ sung Vitamin C và uống nước mát." },
    ...
    { name: "Đau đầu do căng thẳng", group: "Thần kinh", keywords: ["đau đầu", "nhức đầu", "căng đầu"], risk: "Thấp", advice: "Nghỉ ngơi, rời xa màn hình máy tính." },
    ...
    { name: "Stress học đường", group: "Tâm lý", keywords: ["căng thẳng", "mệt mỏi", "khó tập trung", "áp lực"], risk: "Trung bình", advice: "Sắp xếp lại lịch học, đi dạo nhẹ nhàng." },
    ...
    { name: "Hồi hộp/Lo âu", group: "Tâm lý", keywords: ["tim đập nhanh", "run rẩy"], risk: "Thấp", advice: "Tập hít thở sâu theo nhịp 4-7-8." },
    ...
    { name: "Mụn trứng cá", group: "Da liễu", keywords: ["mụn", "nổi mụn"], risk: "Thấp", advice: "Rửa mặt sạch và không tự ý nặn mụn." },
    { name: "Mề đay", group: "Da liễu", keywords: ["ngứa da", "nổi sẩn"], risk: "Trung bình", advice: "Tìm nguyên nhân gây dị ứng để phòng tránh." },
    { name: "Cháy nắng", group: "Da liễu", keywords: ["da đỏ nắng", "rát da"], risk: "Thấp", advice: "Sử dụng gel lô hội để làm dịu da." },
    { name: "Hắc lào", group: "Da liễu", keywords: ["ngứa vòng tròn", "lác đồng tiền"], risk: "Trung bình", advice: "Giữ da khô ráo và bôi thuốc trị nấm." },
    { name: "Lang ben", group: "Da liễu", keywords: ["đốm trắng", "loang lổ"], risk: "Thấp", advice: "Tắm rửa sạch sẽ, tránh dùng chung khăn tắm." },
    { name: "Rôm sảy", group: "Da liễu", keywords: ["hạt đỏ ngứa", "mẩn ngứa nóng"], risk: "Thấp", advice: "Mặc đồ thoáng mát và tắm nước lá." },
    { name: "Kiến ba khoang", group: "Da liễu", keywords: ["phỏng da", "rát da kiến", "ba khoang"], risk: "Trung bình", advice: "Rửa bằng nước sạch, bôi thuốc mỡ làm dịu." },
    { name: "Ghẻ", group: "Da liễu", keywords: ["ngứa kẽ tay", "ngứa đêm"], risk: "Trung bình", advice: "Vệ sinh giường chiếu, dùng thuốc bôi chuyên dụng." },
    { name: "Mụn cóc", group: "Da liễu", keywords: ["mụn cứng tay", "mụn ké"], risk: "Thấp", advice: "Đến da liễu để xử lý bằng nitơ lỏng." },

    /* ================= TRUYỀN NHIỄM & DINH DƯỠNG (96-100) ================= */
    ...
    { name: "Sốt xuất huyết", group: "Truyền nhiễm", keywords: ["sốt cao liên tục", "phát ban đỏ", "xuất huyết"], risk: "Rất cao", advice: "Đi xét nghiệm máu ngay tại bệnh viện." },
    { name: "Thủy đậu", group: "Truyền nhiễm", keywords: ["mụn nước", "ngứa toàn thân", "trái rạ"], risk: "Trung bình", advice: "Tránh gãi vỡ mụn và bôi thuốc xanh." },
...
function checkSymptoms() {
    ...
    // 2. Hiển thị Loading (AI đang suy nghĩ)
    const loadingId = "loading-" + Date.now();
    appendMessage("ai", `<div class="typing-indicator" id="${loadingId}"><span></span><span></span><span></span></div>`);

    // 3. Giả lập độ trễ của AI (800ms) để tạo cảm giác thật
    setTimeout(() => {
        // Xóa loading
        const loadingElement = document.getElementById(loadingId);
        if (loadingElement) loadingElement.closest('.message-wrapper').remove();

        const inputLow = userText.toLowerCase();
        
        // Thuật toán Score: Tính toán độ khớp
        let results = [];
        diseasesDatabase.forEach(disease => {
            let score = 0;
            disease.keywords.forEach(kw => {
                if (inputLow.includes(kw)) {
                    score += kw.length; // Keyword càng dài độ chính xác càng cao
                }
            });
        ...
        // 4. Phản hồi
        if (results.length > 0) {
            let responseHtml = `<p><b>HealthScan</b> đã phân tích các dấu hiệu của bạn:</p>`;
            
            // Chỉ lấy tối đa 3 kết quả sát nhất
            results.slice(0, 3).forEach((d, index) => {
                const colors = getRiskStyles(d.risk);
                const star = index === 0 ? "⭐ " : "";
                
                responseHtml += `
                    <div class="disease-card" style="border-left: 5px solid ${colors.main};">
                        <div class="card-header-flex">
                            <strong style="color: ${colors.main};">${star}${d.name}</strong>
                            <span class="risk-badge" style="background: ${colors.bg}; color: ${colors.main}">${d.risk}</span>
                        </div>
                        <small class="group-tag">Nhóm: ${d.group}</small>
                        <p class="advice-box"><b>💡 Lời khuyên:</b> ${d.advice}</p>
                    </div>`;
            });

            appendMessage("ai", responseHtml);
        } else {
            appendMessage("ai", "Hmm... HealthScan chưa nhận diện được triệu chứng này. Bạn hãy thử mô tả cụ thể hơn như 'đau bụng phải' hoặc 'sốt cao' nhé?");
        }
    }, 1000);
}

/**
 * Trả về màu sắc dựa trên mức độ rủi ro
 */
function getRiskStyles(risk) {
    switch (risk) {
        case "Rất cao": return { main: "#ff4757", bg: "#ffeef0" };
        case "Cao":     return { main: "#e67e22", bg: "#fef5ed" };
        case "Trung bình": return { main: "#f1c40f", bg: "#fefced" };
        default:        return { main: "#2ecc71", bg: "#ebf9f1" };
    }
}

/**
 * Hàm hiển thị tin nhắn vào ChatBox
 */
function appendMessage(sender, content) {
    const chatBox = document.getElementById("chatBox");
    const wrapper = document.createElement("div");
    wrapper.className = `message-wrapper ${sender}-wrapper pulse-in`;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    input.addEventListener("focus", () => {
        document.querySelector(".input-container").style.borderColor = "#2ecc71";
    });

    input.addEventListener("blur", () => {
        document.querySelector(".input-container").style.borderColor = "#eee";
    });
});
